...
You also allowed using current version of the library in non-GPL projects for free (see license terms for details or contact us at sales@dhtmlx.com)
...
dhx.DataLoader=dhx.proto({
	...
	loadNext:function(count, start){
		if (dhx.isNotDefined(start)) start = this.dataCount();
		if (this.callEvent("onDataRequest", [start,count]) && this.data.feed)
			this.data.feed.call(this, start, count);
	},
	//default after loading callback
...
dhx.DataRecord = dhx.proto({
	...
	refresh:function(){ this.callEvent("onBindRequest"); }
}, dhx.EventSystem, dhx.BaseBind, dhx.AtomDataLoader);
...
ui._view = function(){
	...
	dhx.styles.viewBorder = "1px solid "+color;
	dhx.styles.viewBorder0 = "0px solid "+color;
...
dhx.protoUI({
	...
	resize:function(force){
		...
		if (arguments.length == 2){
			var changed = false;
			if (x>0 && this._last_size[0] != x || force){
				this._settings.width = x;
				var changed = true;
			}
			if (y>0 && this._last_size[1] != y || force){
		...
		if (parent){
			...
			this.$setSize((x||this._last_size[0]),(y||this._last_size[1]),true);
			return false;
		}
		return true;
	}
...
dhx.protoUI({
	...
	setValue:function(value){
		...
		if (this._dataobj.childNodes.length > 0){
			...
			/*if (this._settings.change){
				var code = dhx.toFunctor(this._settings.change);
				if (code && code.call) code.call(this, this._settings.id, value, oldvalue);
			}*/
...
dhx.Values = {
	...
	setValues:function(data){
		...
		for (var name in this.elements)
			if (this.elements[name] && !dhx.isNotDefined(data[name]))
...
dhx.TreeStore = {
	...
	*/
	...
	groupBy: function(_scheme, pull, inner_call, parent, level) {
		if (inner_call === true) {
			// inner call
			var tree = {};
			...
			this.groupBy(scheme, this.pull, true, '0', 0);
			this.refresh();
		}
	}
};
...
dhx.chart.pie = {
	...
	*/
	_renderPie:function(ctx,data,point0,point1,ky,map){
		...
		for(var i = 0; i < data.length;i++){
			values[i] = parseFloat(this._settings.value(data[i])||0);
...
dhx.protoUI({
	...
	_set_inner_size:function(){
		...
		for (var i = 0; i < opts.length; i++){
            if (!this._settings.stepless)
      		...
      		this._lefts[opts[i].value] = 5+this._slider_step*i;
    ...
    _reflectValue:function(left){
    	...
    	box.childNodes[2].style.left = left+"px";
    	box.childNodes[2].style.width = this._bar_width-left+7+"px";
    },
...
/*DHX:Depend libs/proxy.js*/
/*DHX:Depend core/dhx.js*/
/*DHX:Depend core/load.js*/
/*DHX:Depend ui/view.js*/
/*DHX:Depend libs/dataprocessor.js*/
/*DHX:Depend libs/storage.js*/
if(!window.dhx)
	dhx = {};
...
dhx.proxy = dhx.proto({
	...
	$init: function(config) {
		...
		this.$proxy = true;
		this.$progress = false;
		if (dhx.DataProcessor) {
			this._updatesToParams = dhx.bind(dhx.DataProcessor.prototype._updatesToParams, this);
			this._updatesData = dhx.bind(dhx.DataProcessor.prototype._updatesData, this);
		}
		this.$ready.push(this._after_init_call);
		this._dests = [];
	},
...
//		this._to_storage({'dp': {}});
	...
	_repeat: function() {
		...
		if (typeof(data[0]) !== 'undefined') {
			...
			this[method](this._settings.url, data, {
